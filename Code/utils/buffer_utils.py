...
FinetuneBatch = collections.namedtuple(
    "FinetuneBatch",
    ["observations", "actions", "rewards", "discounts", "next_observations", "embeddings", "prev_embeddings"])


MaskBatch = collections.namedtuple(
    "MaskBatch",
    ["observations", "actions", "rewards", "discounts", "next_observations", "embeddings", "masks", "prev_embeddings"])
...
EmbeddingBatch = collections.namedtuple(
    "EmbeddingBatch",
    ["pos_embeddings", "neg_embeddings", "lag_embeddings", "prev_pos_embeddings", "prev_neg_embeddings", "prev_lag_embeddings"])
...
class DistanceBuffer:
    ...
    def sample_with_mask(self, batch_size: int, temp_coef: int, l2_margin: float = 0.05) -> Batch:
        idx = np.random.randint(0, self.size, size=batch_size)
        prev_idx = (idx - temp_coef) % self.size
        ...
        batch = MaskBatch(observations=self.observations[idx],
                          ...
                          masks=masks,
                          prev_embeddings=self.embeddings[prev_idx])

        return batch

    def sample(self, batch_size: int, temp_coef: int) -> Batch:
        idx = np.random.randint(0, self.size, size=batch_size)
        prev_idx = (idx - temp_coef) % self.size
...
class EmbeddingBuffer:
    ...
    def sample(self, batch_size, temp_coef):
        ...
        prev_pos_idx = (pos_idx - temp_coef) % self.valid_size
        prev_lag_idx = (lag_idx - temp_coef) % self.valid_size
        prev_neg_idx = (neg_idx - temp_coef) % self.valid_size

        return EmbeddingBatch(pos_embeddings=self.pos_embeddings[pos_idx],
                              lag_embeddings=self.pos_embeddings[lag_idx],
                              neg_embeddings=self.neg_embeddings[neg_idx],
                              prev_pos_embeddings=self.pos_embeddings[prev_pos_idx],
                              prev_lag_embeddings=self.pos_embeddings[prev_lag_idx],
                              prev_neg_embeddings=self.neg_embeddings[prev_neg_idx])
